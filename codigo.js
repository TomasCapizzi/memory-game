...
const fondo = {
    id: 0,
    nombre: 'fondo',
    src: 'https://i.pinimg.com/originals/93/b5/f9/93b5f9913d2e4316cd6e541c67b9aed0.jpg'
}
    ...
    for (let i = 0; i < posibilidades; i++) {
        ...
        setTimeout(() => {
            carta.addEventListener('click', () => { // evento click al clickear imagen
                ...
                guardarElección(cartas[i]); //Guardamo eleccion
                if ((eleccion.length % 2) == 0 && eleccion.length > 1) {
                    controlarResultado(eleccion[0].id, eleccion[1].id, carta);
...
function controlarResultado(valor1, valor2, carta) {
    // console.log(valor1, valor2)

    if (opcionesCorrectas < 5) { // Ejecuta hasta antes de adivinar la última
        if (valor1 == valor2) { // Si los valores elegidos son iguales
